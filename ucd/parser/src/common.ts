import { anyCharOf, hex, lower, noCharOf, string, upper, type Parjser } from "parjs";
import {
    between,
    many,
    manySepBy,
...
const spaces = anyCharOf("\t ").pipe(many(), stringify());
...
export const pGeneralCategory = upper().pipe(then(lower()), stringify(), between(spaces));
