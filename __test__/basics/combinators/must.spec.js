...
describe("must combinators", () => {
    describe("must combinator", () => {
        let parser = parsers_1.Parjs.stringLen(3).must(s => s === "abc", "must be 'abc'", result_1.ReplyKind.FatalFail);
        ...
        it("fails accordingly if it doesn't match the condition", () => {
            custom_matchers_1.expectFailure(parser.parse("abd"), result_1.ReplyKind.FatalFail);
        });
    });
    it("mustBeOf", () => {
    ...
    describe("mustCapture combinator", () => {
        let parser = parsers_1.Parjs.string("a").then(parsers_1.Parjs.string("b")).str.or(parsers_1.Parjs.eof.result("")).mustCapture(result_1.ReplyKind.FatalFail);
        ...
        it("fails accordingly if it succeeds but doesn't capture", () => {
            custom_matchers_1.expectFailure(parser.parse(""), result_1.ReplyKind.FatalFail);
        });
    });
    describe("mustBeNonEmpty combinator", () => {
        ...
        let fail = parsers_1.Parjs.fail("", result_1.ReplyKind.FatalFail);
        let emptyObj = parsers_1.Parjs.result({});
        it("fails for empty string", () => {
            custom_matchers_1.expectFailure(emptyString.mustBeNonEmpty.parse(""), result_1.ReplyKind.HardFail);
        });
        it("fails for empty array", () => {
            custom_matchers_1.expectFailure(emptyArray.mustBeNonEmpty.parse(""), result_1.ReplyKind.HardFail);
        });
        it("succeeds for 0 result", () => {
            custom_matchers_1.expectSuccess(zeroResult.mustBeNonEmpty.parse(""), 0);
        });
        it("fails for undefined", () => {
            custom_matchers_1.expectFailure(emptyUndefined.mustBeNonEmpty.parse(""), result_1.ReplyKind.HardFail);
        });
        it("fails for null", () => {
            custom_matchers_1.expectFailure(emptyNull.mustBeNonEmpty.parse(""), result_1.ReplyKind.HardFail);
        });
        it("fails for fail", () => {
            custom_matchers_1.expectFailure(fail.mustBeNonEmpty.parse(""), result_1.ReplyKind.FatalFail);
        });
        it("fails for empty object", () => {
            custom_matchers_1.expectFailure(emptyObj.mustBeNonEmpty.parse(""), result_1.ReplyKind.HardFail);
        });
    });
});
