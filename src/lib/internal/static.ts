...
import {
    ...
    PrsStringLen,
    PrsStringOf
} from "./implementation/parsers";
import {PrsAlternatives, PrsLate, PrsSequence} from "./implementation/combinators";
...
import {ParjsStatic, ParjsStaticHelper, ParserSpecification} from "../parjs";
...
export class ParjsParsers implements ParjsStatic {
    ...
    seqObject(...specs: ParserSpecification<any>[]): LoudParser<any> {
        let parsers = [];
        let keys = [];
        for (let spec of specs) {
            for (let key in spec) {
                if (!Object.prototype.hasOwnProperty.call(spec, key)) continue;
                parsers.push(spec[key]);
                keys.push(key);
            }
        }

        return this.seq(...parsers).map(arr => {
            let obj = {} as any;
