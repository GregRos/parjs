...
export function isHexCode(code: number) {
    return (
        (code >= AsciiCodes.A && code <= AsciiCodes.F) ||
        (code >= AsciiCodes.a && code <= AsciiCodes.f) ||
...
export function isDigitCode(code: number, base = 10) {
    const baseDif = base - 10;
    if (baseDif <= 0) {
        return code >= AsciiCodes.zero && code <= AsciiCodes.nine + baseDif;
    } else {
        return (
            (code >= AsciiCodes.zero && code <= AsciiCodes.nine) ||
            (code >= AsciiCodes.a && code < AsciiCodes.a + baseDif)
        );
    }
}
...
/**
 * Returns true if `code` is a word character, which is a digit, a letter, dash or underscore:
 * `[0-9a-zA-Z_-]`.
 *
 * @param code The character code.
 */
...
/**
 * Returns true if `char` begins with a word char: a letter, digit, underscore, or dash:
 * `[a-zA-Z0-9_-].
 *
 * @param char A string representing a character.
 */
