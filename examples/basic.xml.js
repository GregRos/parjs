...
let ident = src_1.Parjs.letter.then(src_1.Parjs.digit.or(src_1.Parjs.letter).many()).str;
...
let closeTag = ident.between(src_1.Parjs.string("</"), src_1.Parjs.string(">"))
    .must((result, userState) => result === last(userState.tags).tag)
    .act((result, userState) => {
    let topTag = userState.tags.pop();
    last(userState.tags).content.push(topTag);
}).q;
